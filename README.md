...
3. On all of the codes, a whole line is read, an input is extracted and the rest of the line is discarded. This is not aimed at continuous extraction of data.
...
// read_line and read_input defined here
```
...
### Function templates (with error checking)

Adding error check on our previus template simply requires setting the return type as `bool` and doing `return !stream.fail();` at the end.<br>
The source is edited to now use if statements:
...
See the [full source here](function-templates-with-ec/main.cpp).
...
- **Error checking**<br>
  Now we can check for errors when needed
